...
const ProgressOverview = () => {
  ...
  useEffect(() => {
    const getSavedEveryDayTask = JSON.parse(
      localStorage.getItem("everydayTaskData")
    );
  ...
  useEffect(() => {
    const handlerUpdateEveryDayTask = () => {
      const updateSavedEveryDayTask = JSON.parse(
        localStorage.getItem("everydayTaskData")
      );
    ...
    <div className="ProgressOverview">
      <div className="during_progress">
        ...
        <BsQuestionLg
          ...
          onClick={() => setShowMeLineChartDetails((prev) => !prev)}
        />

        {showMeLineChartDetails ? (
          <div className="lineChartDetails">
            <p>
              Here, we calculate the daily task quality by taking the completed
              tasks for the day and dividing it by the total tasks assigned. We
              then multiply this result by 100 to obtain a percentage, giving
              you insights into the quality of your daily progress.
            </p>
          </div>
        ) : null}
      </div>
      <div className="progress_skills">
        <ProgressSkills />
        <BsQuestionLg
          onClick={() => setShowMeSkillsPieDetails((prev) => !prev)}
        />
        {showMeSkillsPieDetails ? (
          <div className="skillsPieDetails">
            <p>
              Discover the Top Seven Learning Topics Derived from Recent
              Activity. formula: (topic's duration / topic's amount)
            </p>
          </div>
        ) : null}
      </div>
