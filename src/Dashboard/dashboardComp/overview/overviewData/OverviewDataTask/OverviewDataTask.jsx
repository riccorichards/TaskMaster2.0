...
const OverviewDataTask = () => {
  ...
  console.log(showWeeklyTaskData);
  useEffect(() => {
  ...
  const totalDailyTask =
    showDailyTaskData.length > 0
      ? showDailyTaskData.reduce((acc, obj) => {
          return acc + obj.tasksInDay.length;
        }, 0)
      : null;

  const totalCompleteDailyTasks =
    showDailyTaskData.length > 0
      ? showDailyTaskData.reduce((acc, obj) => {
          const complete = obj.tasksInDay.filter(
  ...
  const totalWeeklyCompleteTask = showWeeklyTaskData
    .flat()
    .filter((task) => task.complete === true).length;
    ...
    <div className="OverviewDataTask">
      ...
      {showMeTaskDetails ? (
        <div className="taskDetails">
          <p>
            Here, we've implemented a calculation to determine progress by
            dividing the number of completed tasks by the total tasks, then
            multiplying it by 100 to obtain a percentage value. This allows you
            to easily track and visualize your achievements within the project.
          </p>
        </div>
      ) : null}
    </div>
