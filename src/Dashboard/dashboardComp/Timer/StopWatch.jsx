...
const StopWatch = () => {
  ...
  useEffect(() => {
    const getDataForRecently = JSON.parse(
      localStorage.getItem("dataForRecently")
    );
    ...
    series: [
      {
        ...
        data: [{ value: maxWorkingTime - timeValue }, { value: timeValue }],
  ...
  return (
    <div className="stopWatch_wrapper">
      ...
      <button
        className="showWorkSpaceInput"
        onClick={() => setShowWorkSpaceInput((prev) => !prev)}
      >
        Time For
      </button>
      {showWorkSpaceInput ? (
      ...
      <div className="stopWatch_controler">
        {!runStopWatch ? (
          <AiFillPlayCircle onClick={() => setRunStopWatch(true)} />
        ) : (
          <BsFillPauseCircleFill onClick={() => pauseStopWatch()} />
        )}
